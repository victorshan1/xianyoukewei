...
- 天气、空气质量、逐小时预报和多日预报使用 `window.lingguang.data.getWeather`，开发前阅读 `docs/API_DATA_WEATHER.md`。
- 法币汇率和金额换算使用 `window.lingguang.data.getExchangeRate`，开发前阅读 `docs/API_DATA_EXCHANGE_RATE.md`。
- A 股、港股和美股的最新行情使用 `window.lingguang.data.getStockPrice`，开发前阅读 `docs/API_DATA_STOCK_PRICE.md`。
- A 股、港股和美股的日线、周线和月线数据使用 `window.lingguang.data.getStockCandlestickData`，开发前阅读 `docs/API_DATA_STOCK_CANDLESTICK.md`。
- 上海黄金交易所和国际贵金属行情使用 `window.lingguang.data.getPreciousMetalPrice`，开发前阅读 `docs/API_DATA_PRECIOUS_METAL_PRICE.md`。
- 上述领域之外的公开数据使用 `window.lingguang.data.fetch`，具体契约见 `docs/API_DATAFETCH.md`。
